...
def get_vars_out_paths(
    RESULT_FOLDER: Path, var_search: list[str]
) -> Dict[str, list[Path]]:
    """
    ...
    for var in var_search:
        varname = (
            var[:-1] if var.endswith("_") else var
        )  # Remove trailing _ if they exist
...
def load_array(var_XXXXX: Path, Mglob: int, Nglob: int):
    ...
    try:
        ...
        if var_XXXXX.name == "time_dt.out":
            return np.loadtxt(var_XXXXX, dtype=np.float64)
        elif var_XXXXX.name.startswith("sta_"):
            return np.loadtxt(var_XXXXX, dtype=np.float64)

        # Everything else must be binary
        else:
            return np.fromfile(var_XXXXX, dtype=np.float64).reshape(Nglob, Mglob)
    ...
    # Pad with zeros otherwise if error
        ...
        return np.zeros((Nglob, Mglob), dtype=np.float64)
...
def get_into_netcdf():
    ...
    print("HERE")  # Acess necessary paths
    ...
    for var_name, var_value in output_variables.items():
        ...
        if var_value.ndim == 3 and var_value.shape == (t_FW.size, Nglob, Mglob):
            ...
            var_value.ndim == 2 and Nglob == 1 and var_value.shape == (t_FW.size, Mglob)
        ):
    ...
    print(f"FILE PATH: {ptr['nc']}")
    # Save to netcdf
    ds.to_netcdf(ptr["nc"], mode="w", encoding=encoding)  # , engine="scipy")
