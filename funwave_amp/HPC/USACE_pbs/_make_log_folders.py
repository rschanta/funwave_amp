...
def make_log_folders(log_dir, all_pbs_flags):
    ...
    # If array, need to do PBS flags with % and add an extra folder layer for clarity
        ...
        # Replaced $PBS_ARRAY_INDEX with ^array_index^, a PBS specfic variable
        all_pbs_flags["-o"] = os.path.join(out_dir, "out_^array_index^.out")
        all_pbs_flags["-e"] = os.path.join(err_dir, "err_^array_index^.out")
        # Echo results
