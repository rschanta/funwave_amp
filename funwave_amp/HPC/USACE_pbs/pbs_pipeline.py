...
class PBS_Pipeline:
    ...
    def __init__(self, pbs_vars=None, env=None):
    ...
    def _add_job(self, script_content_func, dep_flags={}, **kwargs):
        # pbs edit parameters
        pbs_edit = kwargs.pop("pbs_edit", {})

        # pbs Flags
        all_pbs_flags = {
            **self.pbs_vars,  # Default flags
            **pbs_edit,  # Edited flags
            **dep_flags,
        }  # Dependency flags

        # Make log folders, set/edit pbs flags as needed
        all_pbs_flags = make_log_folders(self.log_dir, all_pbs_flags)

        ## Body of Script
        script_body = script_content_func(**kwargs)

        # Write the pbs script
        script = write_pbs_script(self.batch_dir, all_pbs_flags, script_body)
        ...
        try:
            ...
            print(
                f"Submission failed. Making pbs script {all_pbs_flags['-N']} but not submitting... "
            )
            job_id = None
    ...
    def run_pipeline(self, steps):
        ...
        for step_func, kwargs in steps.items():
            ...
            # If this is a dependent script, also need last job_id
                ...
                dep_flags = {"-W depend=afterok": previous_job_id, "-r": "y"}
